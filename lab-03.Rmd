---
title: "Lab 03 - Take a sad plot and make it better"
author: "vivi, Siddhi, Marcel, Maggie, Hanna and Sky - Team name"
date: "`r Sys.Date()`"
output: html_document
---

> ### Note from Sky \
I fell ill, so unfortuantely I cannot make it to lab today. \
If you need the host of the repository to be present feel free to make a new repository and use that instead of this one (since I'm hosting this repository). \
 Have a lovely day. \
<p style='text-align: right;'> _11 Oct 2024_ </p>

### Load packages

```{r load-packages, message = FALSE, warning=FALSE}
library(tidyverse)
library(ggplot2)
```


# Questioning data visualisations

## Image 1

```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("img/Speaking-the-world.jpg")
```

_Write your comments about image 1 below_





## Image 2

```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("img/rose-pop-density-total-area-bw.jpg")
```

_Write your comments about image 2 below_




## Image 3

```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("img/life-expectancy-rule-of-law.png")
```

_Write your comments about image 3 below_




## Image 4

```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("img/global-income-dist.PNG")
```

_Write your comments about image 4 below_





# Exercises

```{r load-data-staff, message=FALSE, eval=TRUE}
staff <- read_csv("data/instructional-staff.csv")
```


## Exercise 1

What is the difference between *wide format* data and *long format* data?

A wide format has values that do not repeat in the first column. A long format has values that do repeat in the first column.


Is the format `staff` data *wide* or *long*?

wide

How many observations are there in the data set?

11


## Exercise 2

```{r pivoting}


staff_long <- staff %>%
  pivot_longer(
    cols = !faculty_type, 
    names_to = "year",
    values_to = "percent"
    )
print(staff_long)
```


## Exercise 3

```{r dot-plot}
ggplot(data = staff_long,
       mapping = aes(x = year, 
                     y = percent, 
                     colour = faculty_type)) +
  geom_point()




```

## Exercise 4

```{r bad-line-plot}
ggplot(data = staff_long,
       mapping = aes(x = year, 
                     y = percent, 
                     colour = faculty_type)) +
  geom_line()



```

## Exercise 5

```{r mutate-year}

staff_long <- staff_long %>%
  mutate(year = as.numeric(year))


```


```{r working-line-plot}
ggplot(data = staff_long,
       mapping = aes(x = year, 
                     y = percent, 
                     colour = faculty_type)) +
  geom_line()




```

## Exercise 6

```{r better-line-plot}
ggplot(data = staff_long,
       mapping = aes(x = year, 
                     y = percent, 
                     colour = faculty_type)) +
  geom_line() +
  labs(title = "Year percentage of the faculties",
       x = "Years",
       y = "Percentage(%)",
       colour = "Faculty")




```

## Exercise 7

_Write below your ideas for adapting the line plot_

* Idea 1
* Idea 2
* Idea 3
* ...


## Exercise 8

```{r implement-Ex7}
# Write your code for Exercise 8 here



```

